...
inline void PerfectMatching::ProcessSelfloop(Node* b, Edge* a)
{
	...
	if (prev[0] != prev[1])
	{
		ADD_EDGE(prev[0], a, 1);
		ADD_EDGE(prev[1], a, 0);
		a->slack -= 2*ARC_TO_EDGE_PTR(prev[0]->blossom_sibling)->blossom_eps;
	}
	else
	{
...
void PerfectMatching::Expand(Node* b)
{
	...
	if (i != k)
	{
		...
		i = ARC_HEAD(k->match);
		for (i_ptr=&ARC_HEAD(b->tree_parent)->first_tree_child; *i_ptr!=i; i_ptr = &(*i_ptr)->tree_sibling) {}
		*i_ptr = j;
		j->tree_sibling = i->tree_sibling;
		i->tree_sibling = NULL;
	}
